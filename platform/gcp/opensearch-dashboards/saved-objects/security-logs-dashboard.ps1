...
# PSA / Calico / Istio는 신뢰 가능한 위반/차단 원천이 검증되기 전까지
# 대시보드 분류와 상세 테이블에서 제외한다.
# ---------------------------------------------------------------------------
$controlsState = @{
  title = "보안 로그 필터"
...
$classificationState = @{
  ...
  params = @{
    type = "histogram"; addTooltip = $true; addLegend = $false; legendPosition = "right"
    # A count comparison does not express severity. Keep all categories in
    # one neutral palette color; filter labels provide the event identity.
    colors = @{
      "Keycloak 인증 실패" = "#6092C0"; "OAuth 인증 실패" = "#6092C0"
      "로그아웃/콜백 오류" = "#6092C0"; "Kyverno 정책 위반" = "#6092C0"
    }
    seriesParams = @(@{
      data = @{ id = "1"; label = "건수" }; type = "histogram"; mode = "normal"
      valueAxis = "ValueAxis-1"; drawLines = $true; showCircles = $false; barWidth = 0.7
    })
    categoryAxes = @(@{
    ...
    valueAxes = @(@{
      id = "ValueAxis-1"; name = "ValueAxis-1"; type = "value"; position = "bottom"; show = $true
      scale = @{ type = "linear"; mode = "normal" }
      labels = @{ show = $true; rotate = 0; filter = $false; truncate = 100 }
      title = @{ text = "건수" }
    })
...
Save-Object -Type visualization -Id "security-logs-event-classification" -Attributes @{
  title = "보안 이상 이벤트 유형별 발생 건수"
  description = "현재 신뢰 가능한 원천으로 구조화된 Keycloak/OAuth2 Proxy/Kyverno 보안 이상 이벤트를 유형별로 비교합니다. PSA/Istio/Calico는 원천 검증 전까지 제외합니다."
  version = 1; uiStateJSON = "{}"
